...
default_config = {}
config = ConfigFile(name="util", default=default_config)

@Cmd('msg', kwargs=True, kwargsbool=['nick', 'force'])
def cmdMsg(obj):
    obj.sess['msg'] = obj.kwargs.get('msg', ' '.join(obj.m[1:]))
    obj.sess['chan'] = obj.kwargs.get('chan', obj.dest).lower()
    obj.sess['nick'] = obj.kwargs.get('nick', False)
    if obj.sess['nick']: obj.msg = "%s: %s" % (obj.nick, obj.sess['msg'])
