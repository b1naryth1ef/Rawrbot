...
class Worker(object):
	...
	def ping(self):
		...
		time.sleep(10)
		if self.waitPing:
			print 'Worker #%s has timed out!' % self.id
			self.quit('Timed out!')
	...
	def parse(self, msg):
		if msg['tag'] == 'READY': 
			self.ready = True
		elif msg['tag'] == 'PONG': self.waitPing = False
		else:
			print msg
...
class Network(object):
	...
	def joinChannel(self, chan):
		chan = chan.replace('#', '')
		self.red.delete('i.%s.chan.%s' % (self.id, chan))
		m = None
		for i in self.workers.values():
			if m == None: m = i
	...
	def addWorker(self, reply):
		...
		for i in c:
			if num <= 0: break
			if self.channels[i] != None: self.channels[i].part(i, msg='Bot Swapping!')
...
class Master(object):
	def boot(self):
		...
		try:
			...
			#for i in self.networks.values():
			#	i.quit("MAYDAY! We're going down!")
			#@TODO Tell another master to take over if we have one		
		finally:
